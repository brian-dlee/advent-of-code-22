...
func main() {
	...
	for round < 20 {
		...
		for i, monkey := range monkeys {
			...
			for _, item := range monkey.Items {
				worry, result := inspect(monkey, item)
				if result {
	...
	final := scores[len(scores)-1] * scores[len(scores)-2]

	println("Monkey business:", final)
}

func applyOperation(op Operation, worry int) int {
	v1, v2 := worry, worry

	if op.Operand1.Name != "old" {
		v1 = op.Operand1.IntValue
	}

	if op.Operand2.Name != "old" {
		v2 = op.Operand2.IntValue
	}
...
func inspect(monkey *Monkey, item int) (int, bool) {
	println("  Monkey inspects an item with a worry level of", item)

	worry := applyOperation(monkey.Operation, item) / 3

	println("    Monkey gets bored with item. Worry level is divided by 3 to", worry)

	result := worry%monkey.Test.Value == 0
...
func getMonkeyStartingItems(text string) ([]int, error) {
	var items []int
