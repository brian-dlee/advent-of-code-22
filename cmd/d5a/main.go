...
func (s *Stack) Push(container string) {
...
func (s *Stack) Pop() string {
	...
	panic(fmt.Errorf("cannot Stack.Pop() - size 0"))
}
...
func (m *CraneMap) ExecuteMovesAndReturnTopString() string {
	...
	for i, move := range m.Moves {
		...
		for j < move.Count {
			log.Default().Printf("executing move %d,%d - Source=%d, SourceCount=%d, Target=%d, TargetCount=%d", i, j, move.Source, len(stacks[move.Source].Containers), move.Target, len(stacks[move.Target].Containers))

			if verbose && i == 0 && j == 0 {
				dump(fmt.Sprintf("before %d,%d - Moving %s from %d to %d", i, j, stacks[move.Source].Peek(), move.Source, move.Target), stacks)
			}

			pop := stacks[move.Source].Pop()
			stacks[move.Target].Push(pop)

			log.Default().Printf("move complete %d,%d - Source=%d, SourceCount=%d, Target=%d, TargetCount=%d, Container=%s", i, j, move.Source, len(stacks[move.Source].Containers), move.Target, len(stacks[move.Source].Containers), pop)

			if verbose {
				dump(fmt.Sprintf("after %d,%d - Moved %s from %d to %d", i, j, pop, move.Source, move.Target), stacks)
			}
